...
module.exports = function (options) {
  ...
  return async function redirectRoute (req, res, next) {
    ...
    let skipRules = excludePattern && excludePattern.test(req.url)

    // we can't skip rules when excludePattern included "/" and there are host redirects
    if (skipRules && (hostRedirects.length === 0 || req.url !== '/')) {
      return next()
    }
